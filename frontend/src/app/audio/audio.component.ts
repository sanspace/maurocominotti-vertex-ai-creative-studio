...
export class AudioComponent {
  ...
  generate() {
    ...
    const request: CreateAudioDto = {
        ...
        negativePrompt: this.selectedModel === 'lyria' ? this.negativePrompt : undefined,
        seed: this.selectedModel === 'lyria' ? this.seed : undefined,
        sampleCount: this.selectedModel === 'lyria' ? this.sampleCount : 1,
        languageCode: this.selectedModel !== 'lyria' ? this.selectedLanguage : undefined,
        voiceName: this.selectedModel !== 'lyria' ? this.selectedVoice : undefined,
    };
      ...
      .subscribe({
        next: (response: MediaItem) => {
          if (response.presignedUrls && response.presignedUrls.length > 0) {
            this.audioUrl = this.sanitizer.bypassSecurityTrustResourceUrl(
              response.presignedUrls[0],
            );
          }
        },
        error: (error: any) => {
