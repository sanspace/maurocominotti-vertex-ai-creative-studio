...
configure_environment() {
    ...
    if [ ! -d "$ENV_DIR" ]; then
        ...
        prompt "What prefix should be used inside the bucket?"; read -p "   Prefix [default value: $BUCKET_PREFIX]: " BUCKET_PREFIX_INPUT < /dev/tty; BUCKET_PREFIX=${BUCKET_PREFIX_INPUT:-$BUCKET_PREFIX}
