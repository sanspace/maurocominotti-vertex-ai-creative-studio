...
class Metadata(BaseModel):
    ...
    target_model: str = Field(default="Veo", description="The target video generation model.")
...
class SceneSetup(BaseModel):
    ...
    temporal_elements: str = Field(default="Modern day, daytime", description="Describes time of day, season, or time-related effects. E.g., 'Golden hour at sunset', 'Hyper-lapse of city traffic'.")

class Subject(BaseModel):
    """Describes the main 'who' or 'what' of the video."""
    main_subject: Annotated[str, Field(description="The primary character, object, or focus of the video. E.g., 'A golden retriever puppy', 'A vintage sports car'.")]
    character_details: Optional[str] = Field(default=None, description="Personality, appearance, or specific features if the subject is a character.")
    key_objects: List[str] = Field(default_factory=list, description="Other crucial objects that are part of the subject or interact with it.")
...
class CameraDirectives(BaseModel):
    ...
    camera_angles: Annotated[List[str], Field(description="List of desired camera angles. E.g., ['low-angle', 'top-down', 'dutch angle'].")]
    camera_movements: Annotated[List[str], Field(description="List of camera movements. E.g., ['static shot', 'slow dolly-in', 'crane shot', 'slow motion'].")]
    lens_and_optical_effects: Optional[str] = Field(default=None, description="Describes lens choice and effects. E.g., '85mm macro lens with a shallow depth of field', 'Anamorphic lens flare'.")
...
class AudioDesign(BaseModel):
    ...
    music_style: Optional[str] = Field(default=None, description="Style of background music. E.g., 'Epic orchestral score', 'Minimalist ambient synth'.")
    key_sound_effects: Optional[str] = Field(default=None, description="Important SFX. E.g., 'The crisp sound of footsteps on gravel', 'Magical chimes'.")
    dialogue_or_narration: Optional[str] = Field(default=None, description="Description of any spoken words. E.g., 'Calm, female narrator', 'No dialogue'.")
